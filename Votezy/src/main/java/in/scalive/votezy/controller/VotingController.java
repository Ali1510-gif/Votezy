...
public class VotingController {
	...
	public ResponseEntity<VoteResponseDTO> castVote(@RequestBody @Valid VoteRequestDTO voteRequest){
		...
		VoteResponseDTO voteResponse=new VoteResponseDTO("Voter casted successfully",true,vote.getVoterId(),vote.getCandidateId());
		return new ResponseEntity<>(voteResponse,HttpStatus.CREATED);
	}
	
	@GetMapping
	public ResponseEntity<List<Vote>> getAllVotes(){
		List<Vote>voteList=this.votingService.getAllVoters();
