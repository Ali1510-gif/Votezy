...
document
  ...
  ?.addEventListener("submit", function (event) {
    ...
    fetch("http://localhost:8081/api/candidates/add", {
      method: "POST",
      headers: {
...
function searchCandidate() {
  ...
  fetch(`http://localhost:8081/api/candidates/${candidateId}`)
    .then((res) => res.json())
    .then((candidate) => {
      Swal.fire({
...
function updateCandidate(candidateId, newName, newParty) {
  fetch(`http://localhost:8081/api/candidates/update/${candidateId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name: newName, party: newParty }),
  })
...
function deleteCandidate(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      fetch(`http://localhost:8081/api/candidates/delete/${id}`, {
        method: "DELETE",
      })
        .then(() => {
